...
|---|---|---|
| Gaming Member Identity Foundation | `gaming_members`/`gaming_admins`, Supabase Auth (email OTP) integration, additive `participants.gaming_member_id` linkage, full Guest/member coexistence | **DEPLOYED. PRODUCTION SMTP RESTORED (2026-08-24). REAL PRODUCTION OTP SIGN-IN PROVEN — first real Gaming Member (`ElPrimerInvestigador`) live.** Guest gameplay confirmed unaffected in production. See `IDENTITY_FOUNDATION_IMPLEMENTATION_RECORD.md`'s "Auth Restoration — SMTP/OTP Production Proving" section. |
| Soccer Predictions | Roster-based prediction gameplay with geolocation-gated venue activations, four-dimension settlement, prize qualification, Gaming XP ledger | **DEPLOYED. LOCAL VALIDATION COMPLETE. AUTH NOW RESTORED (2026-08-24) — AUTHENTICATED PRODUCTION END-TO-END FLOW NOT YET INDEPENDENTLY EXERCISED.** See `SOCCER_PREDICTIONS_IMPLEMENTATION_RECORD.md`. |
...
| Phase | What it delivered | Status |
|---|---|---|
| Soccer Predictions v2 | Replaces the flattened `predicted_goal_minute` integer (which collided first-half-stoppage goals with unrelated ordinary minutes) with a structural `(regulation, stoppage)` pair; regulation-time-only settlement scope; own-goal asymmetric treatment; cancelled/abandoned-Match settlement prohibition; a minimal `correct_dimension_count`/`correct_dimension_keys[]` Experience Summary contract extension; a new `official_goal_events` boundary constraint closing a related data-entry defect found during the acceptance audit | **DEPLOYED. SCHEMA/RPC DIRECTLY VALIDATED LIVE. AUTH NOW RESTORED (2026-08-24) — AUTHENTICATED v2 SETTLEMENT NOT YET INDEPENDENTLY EXERCISED.** See `SOCCER_PREDICTIONS_V2_IMPLEMENTATION_RECORD.md`'s "Production Deployment" section for full evidence. |
...
## Auth Restoration + Auth UX Micro-Correction (2026-08-24)

| Phase | What it delivered | Status |
|---|---|---|
| Auth Restoration | Production SMTP restored (`gaming.urbanohn.com` via Resend, Supabase custom SMTP), OTP email template corrected to expose `{{ .Token }}` — all Founder-managed external configuration, no repository change required | **PROVEN LIVE.** Real Confirm-signup OTP sign-in completed in production; first real Gaming Member created and mapped. See `IDENTITY_FOUNDATION_IMPLEMENTATION_RECORD.md`'s "Auth Restoration — SMTP/OTP Production Proving." |
| Auth UX Micro-Correction | Two presentation-only defects found by the restoration proving: hard-coded "6-digit code" copy (production OTP is actually 8 digits) and a brief unauthenticated flash on page load for already-signed-in members | **DEPLOYED.** `public/urbanoAuth.js` only — no backend, schema, or Auth configuration change. See `IDENTITY_FOUNDATION_IMPLEMENTATION_RECORD.md`'s "Auth UX Micro-Correction." |
| Auth Initialization Flash — Residual Defect + Static First-Paint Correction | The JS-only flash fix above proved insufficient — Founder production testing (normal + hard refresh) still showed the flash; root-caused to static HTML painting before the shared Auth script runs; corrected with a static `visibility:hidden` on the sign-in control across all 7 affected pages | **PRODUCTION_PROVEN_FIXED.** Founder re-tested live: no flash on normal or hard refresh. See `IDENTITY_FOUNDATION_IMPLEMENTATION_RECORD.md`'s "Auth Initialization Flash — Residual Defect + Static First-Paint Correction." |

Commit `d1a1e3408cd3b3c8c19be195ddb13ba74b36207e` (carrying `67563eda962a6b43b025732a7ddcfdc361af1b6e`, the Experience Discovery Slice 1 documentation closure, naturally in ancestry) fast-forward pushed to `origin/main` (`5dcdabb..d1a1e34`) and confirmed deployed via GitHub's Vercel deployment-status check, plus a byte-identical content match between the live production `urbanoAuth.js` and the local candidate.

Real production Confirm-signup OTP proven end to end: Founder signed in with a personal email address, received the code from `URBANO Gaming <noreply@gaming.urbanohn.com>`, completed profile creation as `ElPrimerInvestigador`. Verified directly against the database, not just the UI: `auth.users` shows a genuine confirm-then-sign-in event; `gaming_members` gained exactly one new row, correctly mapped, no duplicate. Production OTP observed as **8 digits**, not the 6 the original design assumed — the UI copy was corrected to length-neutral wording rather than the Auth provider being reconfigured to match a UI assumption.

Authorization boundary re-verified with the first-ever real production anon-role credential: direct RPC calls to `bootstrap_governance_authority_atomically`, `grant_platform_authority_atomically`, `revoke_platform_authority_atomically` all returned `42501 permission denied` — live proof, not schema-catalog inference. `service_role` re-confirmed reaching genuine domain validation on the same RPCs. Unauthenticated `401` re-confirmed live; authenticated-but-ungranted `403` remains structurally proven only (no real bearer token was ever requested from the Founder).

Zero automatic consequence, confirmed by direct count before/after both the Auth Restoration proving and the Auth UX deployment: `authority_grants` = 0, `admin_audit_events` = 0, `gaming_xp_events`/`gaming_xp_rules`/`gaming_category_participation_policy` = 0, `gaming_admins` still present. **PLATFORM GOVERNANCE NOT ACTIVATED. GAMING XP NOT ACTIVATED. Migration `0125` remains unapplied.**

Full live regression post-deployment: homepage, `host.html`, `participant.html`, `trivia-playtest.html`, `quiz-playtest.html`, `poker-host.html`, `poker-table.html`, `soccer-predictions.html`, `leaderboards.html`, `rewards.html`, `GET /api/gaming/config`, `GET /api/gaming/leaderboard`, `GET /api/gaming/predictions/matches` all confirmed healthy — **PRODUCTION REGRESSION PASSED**. The unauthenticated-initialization fix was independently live-proven in a genuinely fresh browser context (no stored Supabase session): button correctly resolved visible, `"Sign in with URBANO"`, no permanent-hidden state. The authenticated-refresh flash fix (an already-signed-in member never seeing the flash) remains source-verified rather than independently live-proven — that specific check depends on the Founder's own already-authenticated browser, which this session correctly never requested access to.

**Remaining, explicitly non-blocking Auth evidence gap.** The returning-user (Magic Link) OTP branch — a subsequent sign-in from an already-confirmed identity, which renders a different email template than today's Confirm-signup flow — has not yet been independently exercised.

Final local validation, re-run after production proving: **641/641** behavioral, **144/144** contract, clean typecheck and build.

## Auth Initialization Flash — Residual Defect + Static First-Paint Correction (2026-08-24)

The Auth UX Micro-Correction's JS-only initialization fix, above, left exactly one item explicitly unproven live: whether an already-authenticated member would actually stop seeing the flash on a real refresh. The Founder tested it directly and it falsified that assumption — both a normal refresh and a hard refresh (`Cmd+Shift+R`) of the already-authenticated `ElPrimerInvestigador` session still briefly showed `"Sign in with URBANO"` before settling on the correct authenticated state. Reproducing on a hard refresh ruled out simple caching as the explanation.

Root cause, re-diagnosed from that evidence rather than assumed: every affected page places its sign-in button markup before a plain, non-`defer` `<script src="/urbanoAuth.js">` tag — the browser can paint the static "Sign in with URBANO" text before the script is even fetched, a window too small to observe on fast local testing but real on production. Corrected by adding a static `style="visibility:hidden"` directly to the button markup in every page using the shared Auth control — a complete inventory re-verified three independent ways: `index.html`, `leaderboards.html`, `rewards.html`, `soccer-predictions.html`, `trivia-playtest.html`, `quiz-playtest.html`, and `predictions-admin.html` (an internal admin surface not previously enumerated). `public/urbanoAuth.js` itself was not modified — its existing hide/reveal logic composes correctly with the new static default.

Deployed as commit `6efadcbd3c58c25767e52f2159ac2894cf775015` (parent `d1a1e3408cd3b3c8c19be195ddb13ba74b36207e`), fast-forward pushed `d1a1e34..6efadcb`, Vercel deployment confirmed and the deployed HTML for all 7 pages confirmed via direct `curl` to already contain the hidden state in the raw response — the strongest available proof, independent of any DOM-timing race.

**Founder re-tested production and confirmed the flash is gone**: using the same already-authenticated session against the newly deployed SHA, both a normal refresh and a hard refresh showed no `"Sign in with URBANO"` at any point. **The residual first-paint flash defect is `PRODUCTION_PROVEN_FIXED`** — the first defect in this Auth Restoration arc actually reproduced live by the Founder, then closed and re-verified live by the Founder, not only locally or structurally.
...
Full live regression, all 14 routes (every Auth-control surface plus `host.html`, `participant.html`, `poker-host.html`, `poker-table.html`, and the three `/api/gaming/*` endpoints): confirmed healthy — **PRODUCTION REGRESSION PASSED**.

**The two-step correction is preserved as real history, not compressed.** The JS-only fix was a genuine, reasonable first attempt, not a wrong idea — it fixed everything it was actually tested against and was only found insufficient by real Founder production evidence, not by any test this repository could construct locally. Returning-user (Magic Link) OTP and the fully-fresh-Incognito test remain open, non-blocking evidence items, unaffected by this correction.

Final local validation, re-run after this second production proving: **641/641** behavioral, **144/144** contract, clean typecheck and build.
