...
## 20. Production Deployment (2026-08-22)

A separate, later, explicitly-authorized gate (the "Production Deployment + Validation Gate") deployed this local implementation. This section documents that gate; it does not retroactively claim §19 above described a deployed state.

**A. Deployed.** Migrations `0114`–`0121` applied via `supabase db push --linked` (dry-run confirmed the exact 8-file inventory first, no seeds, no roles); production migration ceiling is now **`0121`**, confirmed via `supabase migration list --linked` showing local/remote parity through `0121`. Commit `1cb0f2128c1d57b7f59b86f28b5c03075b806727` fast-forward pushed to `origin/main` (`da36912..1cb0f21`) and confirmed live via GitHub's own Vercel deployment-status check for this exact SHA (`state: success`, "Deployment has completed").

**B. OLD_SOURCE_NEW_SCHEMA verified live, not assumed.** With schema already at `0121` and source still `da36912`, an old-shaped 2-argument call to `correct_match_result_atomically` returned `PGRST202` — *"Could not find the function... Perhaps you meant... (p_finalized_by_gaming_member_id, p_match_result_id, p_reason)"* — a clean, zero-mutation, function-resolution-layer rejection. The identical old-shaped call to `finalize_match_result_atomically` resolved correctly via its `default null` and reached real domain logic (`MATCH_RESULT_NOT_FOUND`). This matches the local prediction exactly and closes the one genuine transitional risk named in §18 with direct production evidence rather than inference.

**C. Zero automatic authority activation, confirmed both immediately post-migration and again after the full regression sweep.** The three canonical, non-hierarchical platform authority classes (`OPERATIONAL`, `CONSEQUENTIAL_FINALIZER`, `PRODUCT_GOVERNANCE`) are now live in production via `authority_grants`. `authority_grants` and `admin_audit_events` both remained at zero rows throughout; `gaming_members`, `gaming_admins`, `match_results`, `evaluations`, `experience_summaries`, and `prize_qualifications` all remained at their pre-deployment zero. **PLATFORM GOVERNANCE NOT YET ACTIVATED** — no Founder Gaming Member was created, no bootstrap was performed, no authority was granted. Separately, and distinctly: `gaming_xp_rules`, `gaming_category_participation_policy`, and `gaming_xp_events` also remained at zero — **GAMING XP NOT ACTIVATED**. Deployment capability and authority activation were kept strictly distinct, as instructed; these are two separate activation boundaries, neither collapsed into the other.

**D. Safe A0 infrastructure proving, without fabricating business evidence.** `authority_grants`/`admin_audit_events` table existence and `match_results.finalized_by_gaming_member_id` column existence confirmed by direct query. `bootstrap_governance_authority_atomically` called with a syntactically valid but nonexistent Gaming Member id: passed its own no-Governance-exists guard (true, since none existed) and correctly failed on `GAMING_MEMBER_NOT_FOUND` — proving the full guard chain live without ever inserting a row. `grant_platform_authority_atomically`/`revoke_platform_authority_atomically` both correctly rejected with `GOVERNANCE_AUTHORITY_REQUIRED` against nonexistent actors, proving reachability without creating anything. Real business-evidence proving of the *new* authority check specifically inside `finalize`/`correct` (as opposed to signature compatibility, which *was* proven per §B) remains genuinely pending — it requires a real `match_results` row, which this gate correctly declined to fabricate.

**E. Full live regression, end-to-end against the newly-deployed source and schema together:** Guest Open Response (create→declare capability→join→lock→start→submit→close→reveal→complete) — every step succeeded. Voting and Quiz capability declaration and route liveness confirmed (real validation reached, never a 500). The Quiz/Trivia shared prepared-question pool proven live: a question prepared once was consumed by an ad-hoc Trivia `/start` call reaching `PROMPT_ACTIVE`. Guest Poker (create→join×2→deal) — full cycle succeeded. Soccer Predictions public Match list and the Global Leaderboard both confirmed live and honestly empty.

**F. `gaming_admins` and the A0/A1 boundary, reconfirmed post-deployment.** `gaming_admins` remains at zero rows, its schema untouched; all 14 existing Predictions admin routes remain on the legacy binary gate. `GET /api/gaming/config` remains its identical, unchanged, pre-existing `500` — Auth/SMTP untouched.

**G. Fail-closed operational consequence, explicit and intentional, not a defect.** Because `authority_grants` remains empty, **no one can currently exercise the new `CONSEQUENTIAL_FINALIZER` check inside `finalize`/`correct`** — every real finalize/correct attempt will fail closed until a later, separately-authorized gate bootstraps Governance and grants at least one Finalizer. This interrupts no legitimate workflow today: production holds zero Gaming Members, zero Gaming Admins, and zero Results, and Auth/SMTP remains unconfigured.

**H. Transient infrastructure note.** One local `supabase db reset --local` attempt during this gate's own pre-mutation baseline returned `LegacyDbSetupError` while the `analytics` container was still completing its own post-restart health check; confirmed genuinely transient via an immediate, unmodified retry that succeeded cleanly. Not a defect in this Slice.

**I. Final local validation, re-run after all production work, never against production itself.** `tsc --noEmit` clean; **581/581** behavioral tests; **119/119** contract tests; `npm run build` clean; `git diff --check` clean — identical totals to the pre-deployment baseline, confirming the production work introduced no local regression.
