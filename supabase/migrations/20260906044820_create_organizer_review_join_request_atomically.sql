-- ORGANIZER_REVIEW_JOIN_REQUEST. The one-shot path for a request a
-- captain already REJECTED (REV-015 decision 3 / REV-017 unchanged):
-- exactly one organizer review, whose attributable decision closes the
-- matter for Slice 001 — no further transition is possible once
-- organizer_reviewed_at is set. Distinct from decide_join_request_
-- atomically's own organizer-override path, which acts on a still-
-- REQUESTED (not yet decided at all) request.
