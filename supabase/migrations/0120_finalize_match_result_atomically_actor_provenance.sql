-- Migration: 0120_finalize_match_result_atomically_actor_provenance
-- Admin Control Plane A0 — First Consequential Integration.
--
-- Drop-then-recreate, same precedent as every prior replacement (0091,
-- 0098, 0106). Four behavioral changes over 0106, everything else
-- byte-for-byte unchanged:
--
--   1. A new p_reason parameter (nullable, default null — optional for
--      first finalization per canonical reason policy).
--   2. A CONSEQUENTIAL_FINALIZER authority check against authority_grants
--      (0114), immediately after the initial existence lock and before
--      the already_finalized idempotent-return branch — so an
--      unauthorized caller learns nothing about the Result's state, not
--      even that it is already finalized. This is the first DB-layer
--      authority check this function has ever had; previously it
--      trusted the caller entirely.
--   3. finalized_by_gaming_member_id is now actually persisted (0119) —
--      p_finalized_by_gaming_member_id has been accepted by this
--      function since 0064 and discarded ever since.
--   4. Exactly one FINALIZE_RESULT admin_audit_events row on the real
--      first-finalization branch only — never on the idempotent-return
--      branch, so a replay never duplicates audit history.
--
-- No change to settlement logic, evaluation, progression, or prize-
-- qualification behavior — the entire per-prediction loop is identical
-- to 0106.
...
begin
  ...
  values (
    'FINALIZE_RESULT', 'GAMING_MEMBER', p_finalized_by_gaming_member_id, 'CONSEQUENTIAL_FINALIZER',
