-- REGISTER_FOR_COMPETITION. Member-initiated, PUBLISHED only. Idempotent
-- by construction: a repeat registration for the same (competition,
-- member) returns the original row rather than erroring, mirroring the
-- idempotency-first discipline established elsewhere in this
-- repository (e.g. Pulse's own commit_pulse_setup_atomically).
...
begin
  ...
  if v_state <> 'PUBLISHED' then
    raise exception 'COMPETITION_NOT_PUBLISHED: registration is only open while the competition is PUBLISHED'
      using errcode = 'P0001';
  end if;
