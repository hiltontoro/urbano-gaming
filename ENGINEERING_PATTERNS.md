...
## Active Input Preservation Invariant

**Origin**: a Founder production playtest of Math Duel could not be completed — a typed numeric answer repeatedly disappeared before it could be submitted. Traced to `public/participant.html`'s `renderMathDuelActive()` and `public/host.html`'s `renderDuelExceptionalControls()`, both rebuilding a live editable `<input>` via a full `innerHTML` template assignment on every ~2-second polling tick, regardless of whether anything about the Duel had actually changed. The same defect class had already struck once before, in Host manual point-award entry — worked around by replacing the free-entry field with a fixed-value quick-tap button rather than fixed at the render-architecture level, which is why it recurred in two more places rather than staying fixed.

**The invariant**:

> Background synchronization must never clear, overwrite, replace, blur, reset, or otherwise interfere with valid unsent user input merely because a new authoritative server snapshot was received. While an editable control contains an active local draft, that draft remains locally authoritative until the user submits it, the user explicitly cancels it, the authoritative lifecycle changes such that the input is no longer valid, or another explicit Product rule invalidates the draft. Authoritative server synchronization continues around the editable surface without destroying the draft.

**The pattern**: `applySessionSnapshot()` (both files) calls several render functions unconditionally on every poll response — there is no diffing before deciding whether to re-render at all. That alone is not the defect (see `duel.html`'s own `renderPreparedQueue()`, which rebuilds every poll but contains no editable control — harmless). The defect is specifically: *a render function invoked on every poll tick builds its content via one `innerHTML` template-string assignment that happens to include a live editable node*. Full-string rebuild and reduced-frequency polling both miss the actual fix — a genuinely legitimate state change (an opponent submitting, say) still needs to re-render *something*, and if that render is still a blind full rebuild, in-progress input is destroyed exactly as before, just less often. The fix separates two concerns a single template string had conflated: content that is safe to refresh unconditionally (never mid-edit — labels, counts, opponent names) from content that must not be touched unless it has genuinely changed identity. Two concrete techniques, chosen per situation rather than uniformly:

- **Guard the whole rebuild on an identity check**, when nothing in the block ever changes shape during its own active lifetime — `renderDuelOptionInputs()` already did this correctly (`if (children.length === 0) rebuild`) before this defect was ever found elsewhere; `renderDuelExceptionalControls()` now does the same, keyed on `activeDuel.duelId` specifically (not merely "already has children") since a *new* Duel must still start with a fresh, empty reason, while polls *within* one Duel must not touch it.
- **Split chrome from the editable subtree**, when surrounding content legitimately does need to update while the editable node itself must not — `renderMathDuelActive()` now builds a stable shell once per Duel, updates its text/style pieces unconditionally every poll, and rebuilds only the answer `<input>` itself, guarded by comparing its own `data-ordinal` attribute (read fresh from the live DOM, never cached in a module variable) against the newly-authorized challenge's ordinal.

Neither technique introduces a separate client-side draft-state variable decoupled from the DOM — the live input's own current value and its own `data-ordinal`/`dataset.duelId` attribute are the only state, which is sufficient for both currently-known cases and keeps the fix small. A future editable surface whose "has this genuinely changed" check cannot be expressed this simply (a multi-field form, say) should reach for an explicit local draft variable read back into the rebuilt control's initial value — the same invariant, a different mechanism, chosen because the simpler one doesn't fit.

**Why it matters**: this has now independently recurred three times (Host point-award, Duel exceptional reason, Math Duel answer) across two files and three unrelated features. Each prior fix solved its own instance without naming the underlying rule, which is exactly why it kept recurring instead of being caught by a reviewer checking against a known invariant.

**When to reach for it**: any future render function that (a) is reachable from `applySessionSnapshot()`'s own poll-driven path, directly or indirectly, and (b) builds — or could build — DOM containing an `<input>`, `<textarea>`, `<select>`, or `contenteditable` node via a template-string `innerHTML` assignment. Verify, don't assume, that the specific content in question never changes mid-edit before treating a plain rebuild as safe.
