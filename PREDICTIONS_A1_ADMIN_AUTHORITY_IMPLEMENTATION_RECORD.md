...
**Retired in full at the source-code level**, not merely deprecated — steps 1–4 below are unconditional and were completed and deployed in this Slice. Step 5 (the physical drop) is deliberately **not yet applied to production**; see §20.B/G for why and for the accurate current production state. Local sequence as designed and committed:
...
5. Migration `0125` drops the table — **committed locally, intentionally withheld from production** by the deployment gate; this is cleanup of now-dead legacy schema, not required for A1 Product behavior (steps 1–4 already deliver A1's full admin-authority behavior on their own). `gaming_admins` therefore still physically exists in production as of this writing.
...
No Admin Console UI. No Content Library, Promotions, Physical Competition, Duel, Templates, Level 33, or Chess work. No Gaming XP configuration or activation. No Auth/SMTP configuration. No Platform Governance bootstrap in production. No `FAILURE`-outcome audit events. No fix to the `entered_by_gaming_member_id` staleness (`SHOULD_FIX_LATER`, deliberately not addressed). Nothing staged, committed, pushed, or deployed **at the time this section was written** — see §20 below for what a later, separately-authorized gate did.

## 20. Production Deployment (2026-08-23)

A separate, later, explicitly-authorized gate (the "Production Deployment + Validation Gate") accepted, committed, and deployed this local implementation *through schema `0124` and source only*, deliberately excluding migration `0125`. This section documents that gate; it does not retroactively claim §19 above described a deployed state at the time it was written.

**Read this before touching migration `0125`.** Predictions A1 is fully deployed and complete at schema `0124` — the pending local migration file is not a sign of unfinished work. `gaming_admins` is dead legacy storage, read/written by nothing in the deployed source, retained physically only for rollback safety. `0125` is a separate, later, independently-authorized cleanup action, not a next step to simply push — see the cleanup-gate cross-reference at the end of this section.

**A. Deployed, through `0124` only.** Migrations `0122`–`0124` applied via `supabase db push --linked` (dry-run confirmed the exact 3-file inventory first — `0125` was temporarily relocated out of `supabase/migrations/` before the push and restored immediately after, unedited and unrenumbered). Production migration ceiling is now **`0124`**, confirmed via `supabase migration list --linked` showing local/remote parity through `0124`. Commit `aece367e9bde88cc81478559a9837e70c4ae7cc6` fast-forward pushed to `origin/main` (`1cb0f21..aece367`) and confirmed live via GitHub's own Vercel deployment-status check for this exact SHA (`state: success`, "Deployment has completed"), corroborated live by both new A1 routes resolving 401 rather than 404.

**B. Why `0125` was withheld.** The Supabase CLI was proven empirically — via `--help` inspection of both `db push` and `migration up`, and a live `--dry-run` against production — to have no bounded/target-version flag; it applies every pending migration file it finds. Bundling `0125` into this same push would have applied it before the new source (which no longer references `gaming_admins` at all) was confirmed live, which the preceding readiness gate identified as unsafe: old source's `requireGamingAdmin`/`isGamingAdmin` would hard-fail identically across all 14 legacy routes for the whole window. `0125` remains **unapplied**; `gaming_admins` **still physically exists** in production.

**C. `OLD_SOURCE_SCHEMA_0124` checkpoint, verified live before source push.** With schema at `0124` and source still `1cb0f21`, all public and static Gaming surfaces (homepage, host/participant pages, `predictions-admin.html`, `poker-host.html`/`poker-table.html`, public Predictions match list, Global Leaderboard) were confirmed healthy — **PARTIALLY_COMPATIBLE**, matching the readiness gate's own prior classification, with the already-known caveat that Prize Redemption's new authority check would surface as a generic 500 rather than a clean 403 under old source (currently unreachable: zero `gaming_admins` and zero `authority_grants` rows exist).

**D. Zero automatic authority activation or business-evidence fabrication, confirmed both immediately post-migration and again after full regression.** `authority_grants` and `admin_audit_events` remained at zero rows throughout; `gaming_members`, `matches`, `prize_qualifications`, and every Gaming XP table remained at their pre-deployment zero. **PLATFORM GOVERNANCE NOT ACTIVATED** — no bootstrap performed. **GAMING XP NOT ACTIVATED** — no XP-configuration row exists, and the Global Leaderboard implementation was untouched by this Slice's own diff.

**E. Safe A1 infrastructure proving, without fabricating business evidence.** All three modified/new RPCs (`set_match_activity_classification_atomically`, `set_match_xp_eligibility_atomically`, `redeem_prize_qualification_atomically`) called against a syntactically valid but nonexistent target: each correctly resolved the new signature and failed on the expected application-level `MATCH_NOT_FOUND`/`PRIZE_QUALIFICATION_NOT_FOUND` exception, not a PostgREST signature-mismatch error — proving the new contracts are live without ever reaching a mutation branch. Every admin route, new and migrated, correctly fails closed without a bearer token (401); the authenticated-but-ungranted 403 path was proven locally via real-bearer-token contract tests (production holds zero real Gaming Members to exercise this live). `NEW_SOURCE_SCHEMA_0124` — **SAFE**: every A1 route resolves correctly against this schema, `gaming_admins` coexists with zero runtime coupling (§G), and `authority_grants` is confirmed the sole live platform-authority source.

**F. Full live regression, end-to-end against the newly-deployed source and schema together.** Homepage, host/participant pages, `predictions-admin.html`, `poker-host.html`/`poker-table.html` all healthy; public Predictions match list and Global Leaderboard both live and honestly empty; a nonexistent-session lookup returned a coherent auth-required response, not a crash — **PRODUCTION REGRESSION PASSED**, no regression. `GET /api/gaming/config` remains its identical, unchanged, pre-existing `500` ("browser Supabase configuration not set") — no Auth/SMTP dependency was introduced or touched by this deployment.

**G. `gaming_admins` final state and rollback posture, explicit.** `gaming_admins` still exists and remains unapplied for its own drop, by design. A repository-wide search re-confirmed zero live runtime dependency on it anywhere in `app/`/`lib/` or in any function/trigger/RLS body in migration history. Because it still exists, rollback to `aece367` *or any earlier deployment, including pre-A1*, remains safe. Applying `0125` later will narrow that rollback boundary — the deliberate reason its own drop belongs to a separate, later, explicitly-authorized cleanup gate, not this one.

**H. Documentation.** This section and `PROJECT_STATUS.md`'s own "Predictions A1" entry were both written and left unstaged by this deployment gate — neither was committed.

**I. Final local validation, re-run after all production work, never against production itself.** `tsc --noEmit` clean; **587/587** behavioral tests; **125/125** contract tests; `npm run build` clean — identical totals to the pre-deployment baseline, confirming the production work introduced no local regression.

**Cleanup-gate cross-reference, not designed or executed here.** A future, separately-authorized `gaming_admins` cleanup gate must independently re-verify, fresh, before applying `0125`: A1 source remains healthy in production; no runtime `gaming_admins` reference has reappeared anywhere in `app/`/`lib/`; pre-A1 Vercel rollback is no longer the desired emergency-recovery path (a deliberate operational decision, not an automatic consequence of time passing); and production remains free of any legacy `gaming_admins` population that would need migrating first. Only once all four hold may `0125` be considered for production.
